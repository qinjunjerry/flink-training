 ...
 */
public class TroubledStreamingJobJun {
...
//				.assignTimestampsAndWatermarks(new MeasurementTSExtractor())
				.assignTimestampsAndWatermarks(
						WatermarkStrategy.<FakeKafkaRecord>forBoundedOutOfOrderness(Duration.ofMillis(250))
		...
		SingleOutputStreamOperator<WindowedMeasurements> aggregatedPerLocation = sourceStream
				...
				// Solution 3.2
				.aggregate(new MeasurementAggregatingFunction(),
		...
		env.execute(TroubledStreamingJobJun.class.getSimpleName());
	}
	...
	public static class MeasurementDeserializer extends RichFlatMapFunction<FakeKafkaRecord, JsonNode> {
		...
		public static final Logger LOG = LoggerFactory.getLogger(MeasurementDeserializer.class);
...
//		}
		...
		// Solution 1
		@Override
		public void flatMap(FakeKafkaRecord kafkaRecord, Collector<JsonNode> out) throws Exception {
			try {
	...
	// Solution 3.2
	public static class MeasurementAggregatingFunction
			implements AggregateFunction<JsonNode, Tuple2<Long, Double>, Tuple2<Long, Double>> {
		private static final long serialVersionUID = 1L;

		@Override
		public Tuple2<Long, Double> createAccumulator() {
			// f0: total number of events
			// f1: sum of all measurements
			return new Tuple2<>(0L, 0.0);
		}

		@Override
		public Tuple2<Long, Double> add(JsonNode value, Tuple2<Long, Double> accumulator) {
			return new Tuple2<>(
					accumulator.f0 + 1L,
					accumulator.f1 + Double.parseDouble(value.get("value").asText())
			);
		}
	...
	public static class MeasurementWindowAggregatingFunction
			extends ProcessWindowFunction<Tuple2<Long, Double>, WindowedMeasurements, String, TimeWindow> {
		private static final long serialVersionUID = 1L;
...
//			}
			...
			// Solution 3.2
			Tuple2<Long, Double> aggregateNumbers = input.iterator().next();
			aggregate.setEventsPerWindow(aggregateNumbers.f0);
			aggregate.setSumPerWindow(aggregateNumbers.f1);
